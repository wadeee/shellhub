...
#ssh -p "$remote_port" -i $ssh_key "$remote_user"@"$remote_host" "wget https://download.jetbrains.com.cn/teamcity/TeamCity-2025.03.tar.gz && tar zxf TeamCity-2022.10.2.tar.gz"
...
scp -P "$remote_port" -i $ssh_key ./teamcity.nginx.http.conf "$remote_user"@"$remote_host":/etc/nginx/conf.d/
